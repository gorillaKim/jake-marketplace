...
- `{LogicName}/{LogicName}.spec.md`: 요구사항 명세 (기능, 인터페이스, 제약사항, 성공기준)
- `{LogicName}/{LogicName}.prompt.md`: 이 문서를 생성한 Claude 프롬프트 (재생성/수정 시 참고)
- 코드에서 실제 인터페이스(Props, API, CLI args)를 **Read**하여 "인터페이스 정의" 섹션을 채운다
- 태그: `spec` 필수 + domain 태그 1-3개 (`component`, `api`, `service`, `cli`, `animation`, `chart`, `form` 등)
  - `component` 등 기존 태그 목록에 없는 신규 태그 사용 시 frontmatter 주석에 사유 기록
- 템플릿: `$CLAUDE_PLUGIN_ROOT/templates/techspec.md`, `$CLAUDE_PLUGIN_ROOT/templates/techspec-prompt.md`
