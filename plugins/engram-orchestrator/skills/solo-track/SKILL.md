...
3. session_restore(project_key, mode="agent")             # 광역 caveat (오리엔테이션 → mode='agent')
...
> **벌크/다수 이슈 일괄 진행 가드 (필수)**: 여러 이슈를 한 solo-track 세션에서 직렬 처리할 때도 위 **read #1 `note_list(note_type="comment")` 은 이슈마다 1회 반드시** 수행한다 — "앞 이슈에서 봤으니 생략", "한 번에 승격하니 일괄 스킵" 금지. 벌크 승격 중 per-issue 코멘트 점검을 건너뛰면 특정 이슈에만 달린 사용자 코멘트(질문/지시)를 놓친다(실측 회귀 #2024, 근거 #2053). 질문성 코멘트 발견 시 즉시 답변 + `note_resolve` 후 진행.
...
session_restore(mode="agent") → 활성 프로젝트 매칭 → project_key 확정
...
> **동일 파일 다중 이슈 hunk 귀속 (커밋 없이 여러 이슈 진행 시 필수)**: 커밋 사이 없이 여러 이슈가 같은 파일을 수정하면 `git status` 는 파일 단위로만 누적돼 이슈별 diff 경계가 사라진다. 이때 Step D context note 의 "변경 파일" 은 [README — Demo Gate 의 hunk 귀속 표준](../../README.md#demo-gate-3단-검증) SSOT 형식으로 적는다 — `- <path> → #<thisIssue> hunk: <섹션/심볼/요지> (공유: #<other> 는 <다른 섹션>)`. 단독 파일은 `- <path> (이유)` 그대로. (근거 #2005)

**[EVALUATION] 피드백 기록 (의무)**:
데모 진입 전에 반드시 현재 실행 하네스, 스킬 사용 적절성, Engram 시스템 편의성을 평가하는 `[EVALUATION]` 노트를 `note_add(note_type="evaluation")`로 작성합니다. (engram 코어 P1 — `[EVALUATION]` 접두어는 코어 백필 전 회고 fallback 호환을 위해 병행 유지)

```
note_add(issue_id=N, note_type="evaluation", author="agent", agent_id="main@<sess>-issue<N>",
...
note_add(issue_id=N, note_type="context", author="agent",
         ...
         detail="확인 항목:\n- ...\n변경 파일 (이슈별 hunk 귀속):\n- <path> → #N hunk: <섹션/요지> (공유 파일이면 (공유: #<other> 는 <다른 섹션>) 표기 — README Demo Gate hunk 귀속 표준)\n수동 확인:\n- <칸반 X 동작>\n남은 한계:\n- <있다면>\n증거:\n- git diff: <파일>\n- task_test: <pass/n/a>\n- evaluation_note: added\n스킬/룰: rules=<N>건 적용, skills=<M>건 발동 (<불필요 건수>건 불필요)\n불필요 스킬: <이름 목록 또는 없음>")
...
### Step D.6 — 세션 종료 (session_end · 필수)

**리뷰 패스(Step D.5)가 종결되면 그 결과와 관계없이** `session_end(project_key)` 로 세션을 닫는다. demo 진입 후 세션을 열어둔 채 다음 요청을 무한정 기다리지 않는다 (좀비 세션 / `agent_id` 누적 방지).
...
| Step D.5 결과 | session_end 시점 |
|--------------|-----------------|
| **LGTM** | 리뷰 통과 보고 직후 **즉시** `session_end` → Step E 보고. |
| **CHANGES_REQUESTED — 같은 턴에 바로 수정** | 세션 유지하고 **Step B 로 복귀**. 수정 사이클이 끝나 다시 demo→D.5 에서 LGTM 받은 그 시점에 `session_end`. |
| **CHANGES_REQUESTED — 수정 보류 / 사용자 핸드백** | 피드백 보고 후 **즉시** `session_end` (다음 세션에서 `issue_id` 로 Step A.continue 재개). |

> `session_end` 의 `warnings` 가 비어 있으면 정상 종료. context note 누락·미완료 in_progress task 경고가 뜨면 먼저 처리한 뒤 다시 호출한다.

### Step E — 최종 보고

세션 종료(Step D.6) 후 사용자에게 짧게 보고 + 데스크톱 칸반에서 demo → finished 처리 안내.
...
- **벌크/다수 이슈 일괄 진행 시 per-issue 코멘트 점검 생략 금지** — 여러 이슈를 직렬 처리해도 각 이슈마다 Step A.continue read #1(`note_list(note_type="comment")`)을 1회씩 반드시 수행한다. 일괄 승격이라는 이유로 코멘트 점검을 스킵하면 특정 이슈 전용 사용자 코멘트를 놓친다(회귀 #2024).
