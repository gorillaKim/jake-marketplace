...
```
note_list(issue_id=<issue_id>, note_type="comment", include_resolved=false, mode="agent")
note_list(issue_id=<issue_id>, note_type="caveat", include_resolved=false, mode="agent")
```
...
```
task_list(issue_id=<issue_id>, status="required", mode="agent")  → 처리할 task 목록
```
...
```
task_list(issue_id=<issue_id>, status="required", mode="agent")           → 남은 required task 수
task_test_list(issue_id=<issue_id>)                         → checked 여부
Bash("git status --porcelain | awk '{print $2}'")           → 변경+신규 파일 목록 (untracked 포함)
```

> ⚠️ `git diff --name-only HEAD` 대신 `git status --porcelain` 사용 — 신규(untracked) 파일은 git diff 에 나타나지 않으므로 문서 작성 이슈에서 항상 빈 배열이 됩니다.

**이슈 성격별 git 체크 기준**:

| 이슈 성격 | 파일 변경 요건 |
|----------|-------------|
| 코드 변경 | `git status --porcelain` 결과 1개 이상 필수 |
| 문서 전용 (신규 .md/.yaml 파일 추가) | `git status --porcelain` 으로 신규 파일 실재 확인. 결과 0건이면 "파일 미생성"으로 판단 |
| 설정/메타만 (기록·분석 작업) | git 체크 선택적 — context note 로 대체 가능 |
...
- **작업 중 발견·결정·참조 내용 `note_add` 생략 금지** — Step 2 에서 새 발견/결정이 생길 때마다 즉시 `note_add(discovery|decision|reference)` 로 기록. 나중에 몰아서 쓰는 것 금지.
- **required task 가 1개라도 남은 상태에서 `WORKER_RESULT.status="demo_ready"` 보고 금지** — Step 3 evidence 의 `required_tasks_remaining` 이 0 이어야만 demo_ready 허용.
