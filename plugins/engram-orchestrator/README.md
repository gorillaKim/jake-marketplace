...
- **Agents** (5) — `tools` 필드는 명시 나열 (Claude Code 가 와일드카드 미지원).
  ...
  - `engram-retro` — **스프린트 회고 생성기**. 이슈·노트 전체 분석 → 9섹션 마크다운 문서 생성. 읽기 전용 (상태 전이 없음).
- **Skills** (5)
  ...
  - `sprint-retro` — 스프린트 회고 문서 생성 트리거. engram-retro 를 spawn 하여 9섹션 마크다운 저장 (UC10)
...
**회고 문서 9섹션**: 스프린트 요약 / 완료된 것 / 미완료 / 주요 결정 / 발견·배운 것 / 블로커 분석 / 반복 주의사항 / 재발 방지 / 다음 스프린트 액션 아이템
