...
- **Agents** (6) — `tools` 필드는 명시 나열 (Claude Code 가 와일드카드 미지원).
  ...
  - `engram-reviewer` — **demo 이슈 코드 리뷰어**. 단일/batch 모드 지원. LGTM(승인) 또는 CHANGES_REQUESTED(변경요청) 판정. **UI 이슈면 ui-qa-reviewer 를 spawn 해 브라우저 검증 종합.** demo→finished 는 절대 호출하지 않음 (사용자 전용).
  - `engram-retro` — **스프린트 회고 생성기**. 이슈·노트 전체 분석 → 10섹션 마크다운 문서 생성. 읽기 전용 (상태 전이 없음).
  - `ui-qa-reviewer` — **UI 리뷰어**. 번들된 Playwright MCP 로 대상 URL 을 실제 브라우저에서 열어 스크린샷·접근성·인터랙션을 spec/AC 대비 검증. 로그인 필요 시 사용자에게 요청(자동 로그인 금지).
- **Skills** (7)
  - `intake-as-issue` — 다단계 작업 요청만 이슈화 게이트
  - `mission-plan` — 대형 프로젝트 로드맵 수립 및 미션/에픽/이슈 일괄 계획
  - `team-track` — 팀(leader+worker) 이슈 처리 표준 절차 (solo-track 의 팀 버전): 호출 권한 매트릭스 + Step 절차 + WORKER_RESULT 양식
  ...
  - `ui-test` — 대상 URL/화면을 Playwright MCP 로 UI 검증. ui-qa-reviewer 를 spawn (UC11)
- **MCP servers** (번들, `.mcp.json`)
  - `engram` — `http://127.0.0.1:3456/mcp` (Desktop 앱). 활성화 시 자동 연결. 도구는 표준형 `mcp__engram__*`. 사용자가 같은 endpoint 로 이미 등록(project/user scope)했다면 **endpoint dedup + 상위 스코프 우선**으로 흡수되어 중복·이름변경 없음(실측 확인). 전제: engram Desktop 앱 가동(미가동 시 연결만 끊긴 상태로 표시, 플러그인 로드엔 무해).
  - `playwright` — `npx -y @playwright/mcp@latest`. 활성화 시 자동 시작. UI 테스트/리뷰용. 전제: node/npx.
...
- codex/gemini 가 team-track Step 0~4 만 따르면 됨 — claim/release 안 따져도 leader 가 처리.
...
| Actor | 답변 방법 |
|-------|----------|
| 점유 워커 (claude/codex/gemini) | team-track **Step 2.5 incoming check** 가 트리거. `note_add(comment, agent_id=<self>)` + `note_resolve`. 단 같은 이슈로 다음 leader 사이클에 재 spawn 됐을 때만 가능 (Claude Code 동기 모델 한계). |
...
## UI 테스트 (Playwright MCP 번들)

화면/레이아웃/모달 같은 **UI 작업을 실제 브라우저로 검증**한다.

**구성**
- 플러그인이 `playwright` MCP 서버를 **번들**(`.mcp.json`) → 활성화 시 자동 시작(별도 설치 불필요, 전제: node/npx).
- `ui-qa-reviewer` 에이전트가 Playwright MCP 도구로 navigate→snapshot→screenshot→interaction 검증. 공식 문서상 번들 서버는 "standard MCP tools" 로 노출되며(별도 prefix 규칙 미문서화), 표준 형식 `mcp__playwright__browser_*` 이다(`--plugin-dir` 실측 동일). 에이전트는 `tools` 를 생략해 전체 상속하므로 이름 형식과 무관하게 호출된다.

**사용 경로**
1. **직접** — `ui-test` 스킬: "이 화면 UI 검증해줘" + URL → ui-qa-reviewer spawn → PASS/FAIL 보고.
2. **리뷰 자동 발동** — demo 이슈가 UI 성격(키워드 휴리스틱 + `[UI]`/`target_url` 명시 태그)이고 검증 URL 확보 시, `engram-reviewer` 가 ui-qa-reviewer 를 spawn 해 코드 리뷰 + 브라우저 검증을 종합. solo-track(Step D.5)도 reviewer 경유로 자동 커버.

**로그인 필요 페이지**
- ui-qa-reviewer 는 **자동 로그인하지 않는다**. 인증 벽 감지 시 headed 브라우저로 사용자에게 직접 로그인을 요청(AskUserQuestion)하고, 완료 후 동일 세션으로 재개. 취소 시 UI 검증만 SKIPPED 처리(코드 리뷰는 계속). **자격 증명을 요구·저장하지 않는다.**
