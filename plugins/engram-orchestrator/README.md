...
| 4 | `task_list(req)` / `task_test_list` / `Bash("git diff --name-only HEAD")` 재호출 | **evidence 자체 검증** |
...
2. 정체 이슈마다 중복 caveat 확인 → 없으면 `note_add(caveat, agent_id=engram-leader@<sess>)`.
3. `history_by_agent(agent_id=<해당 워커>)` 로 죽었는지 다른 데서 일하는지 점검.
4. **`AskUserQuestion`** 으로 사용자 액션:
...
5. **release**: `issue_release(125, agent_id=<해당 워커>, transition_to="ready")`. 
   권한 거부 시 `issue_release(..., force=true, agent_id=engram-leader@<sess>)` 강제 회수.
6. **handoff**: 위 release → UC4 의 spawn 사이클을 다른 모델로 재실행.
...
| Step | 호출 | 효과 |
|------|------|------|
| A | `epic_list` → `issue_create` + `task_create x N` + `issue_update(ready)` + `issue_claim` | 분할(압축) + 점유 |
| B | 코드 작업 + `note_add(discovery/decision)` + `task_update(finished, agent_id)` | 작업 + 실시간 기록 |
| C | `task_list(req)` + `task_test_list` + `Bash("git diff --name-only HEAD")` | Demo Gate 자체 검증 |
...
| 측면 | UC2~4 (Hybrid) | UC8 (Solo Track) |
|------|---------------|------------------|
| Actor 수 | 2 (leader + worker) | 1 (메인) |
| evidence 자체 검증 | leader 가 재호출 | ⚠️ 없음 — 사용자가 같은 대화에서 즉시 발견 |
| 라우팅 / 멀티 LLM | 가능 | ❌ |
| Claude Code 호출 비용 | Agent spawn 1+ 회 | 0 회 |
| 적정 작업 크기 | 한 PR 이상 | 1~3 task 한 PR 이하 |

> ⚠️ solo-track 은 leader 의 evidence 검증 안전망이 없습니다. 사용자가 같은 대화에 있어 거짓 보고가 즉시 발견되지만, *"호출한 척"* 사고 가능성은 UC2~4 보다 높습니다. 신중한 작업에는 UC2 (leader claim-and-spawn) 권장.
...
leader.Bash("git diff --name-only HEAD") = []   # 실제
