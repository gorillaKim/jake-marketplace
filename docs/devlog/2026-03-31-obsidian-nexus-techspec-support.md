...
- `agents/docsmith-analyzer.md` 수정 — Phase 1 감지 휴리스틱에 `specs/` 디렉토리 패턴 추가, Phase 2 갭 분석 리포트에 `[specs/]` 카테고리 추가
...
이유: 매 세션마다 반복되는 패턴. critic 에이전트 호출 + 플랜 파일 수정 + 구현 순서가 정형화되어 있어 스킬로 만들면 진입 장벽을 낮추고 리뷰 누락을 방지할 수 있음
-->
